...
def build():
    ...
    pyinstaller_args.extend([
        ...
        '--add-data', './files;files' if target_platform.startswith('win') else './files:files',  # 添加 files 目录
        '--add-data', './VersionHistory.txt;.' if target_platform.startswith('win') else './VersionHistory.txt:.',  # 添加版本历史文件
        '--add-data', './Notice.py;.' if target_platform.startswith('win') else './Notice.py:.',  # 添加 Notice
        '--add-data', './README.md;.' if target_platform.startswith('win') else './README.md:.',  # 添加 README.md 文件
    ])
    ...
    for i in ['./TTHighSpeedDownloader.dll', './TTHighSpeedDownloader.so', './TTHighSpeedDownloader.dylib']:
        if os.path.exists(i):
            ...
            pyinstaller_args.append( f'{i};.' if target_platform.startswith('win') else f'{i}:.')
